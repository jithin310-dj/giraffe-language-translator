import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { lovable } from '@/integrations/lovable/index';
import { useAuth } from '@/hooks/useAuth';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { useToast } from '@/hooks/use-toast';
import giraffeHero from '@/assets/giraffe-hero.png';

const Auth = () => {
  const navigate = useNavigate();
  const { user, loading } = useAuth();
  const { toast } = useToast();

  useEffect(() => {
    if (!loading && user) {
      navigate('/home', { replace: true });
    }
  }, [user, loading, navigate]);

  const handleGoogleSignIn = async () => {
    const { error } = await lovable.auth.signInWithOAuth("google", {
      redirect_uri: window.location.origin,
    });
    if (error) {
      toast({
        title: "Sign-in failed",
        description: error.message || "Could not sign in with Google. Please try again.",
        variant: "destructive",
      });
    }
  };

  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent" />
      </div>
    );
  }

  return (
    <div className="flex min-h-screen flex-col items-center justify-center giraffe-gradient giraffe-pattern px-6 relative overflow-hidden">
      {/* Decorative circles */}
      <div className="absolute top-10 right-10 w-32 h-32 rounded-full bg-accent/10 blur-2xl" />
      <div className="absolute bottom-20 left-5 w-40 h-40 rounded-full bg-secondary/30 blur-3xl" />

      <Card className="w-full max-w-sm border-0 shadow-xl bg-card/80 backdrop-blur-sm rounded-3xl">
        <CardContent className="flex flex-col items-center text-center p-8 gap-5">
          <div className="animate-float">
            <img
              src={giraffeHero}
              alt="LinguaLite Giraffe mascot"
              className="h-24 w-24 drop-shadow-lg"
            />
          </div>

          <div>
            <h1 className="text-2xl font-bold text-foreground font-display">
              Welcome to LinguaLite
            </h1>
            <p className="text-sm text-muted-foreground mt-1">
              Sign in to save your translations across devices
            </p>
          </div>

          <Button
            onClick={handleGoogleSignIn}
            size="lg"
            className="w-full h-14 text-base font-bold rounded-2xl shadow-lg bg-foreground text-background hover:bg-foreground/90 gap-3"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            Continue with Google
          </Button>

          <p className="text-xs text-muted-foreground/60 mt-2">
            No password needed — quick & secure
          </p>
        </CardContent>
      </Card>
    </div>
  );
};

export default Auth;
